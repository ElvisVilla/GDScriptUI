...
func visible(value) -> BaseBuilder:
	if value is Binding:
		value.bind(_get_parent_node(), "visible", func(is_visible):
			...
			_get_parent_node().tree_exiting.connect(func(): value.unbind(_get_parent_node(), "visible")))
	else:
...
func focus() -> BaseBuilder:
	_content_node.call_deferred("grab_focus")
	return self
...
func scale(value) -> BaseBuilder:
	if value is Binding:
		...
		_content_node.tree_exiting.connect(func():
			value.unbind(_content_node, "scale"))
	else:
		_get_parent_node().scale = Vector2(value, value)
...
func _notification(what):
	if what == NOTIFICATION_PREDELETE:
		print("Object being deleted: BaseBuilder")
