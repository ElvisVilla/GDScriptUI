...
func write_code(user_defined_views, view_script_path: String) -> void:
	...
	for view in user_defined_views:
		...
		if not view.has("type") or not view.has("path") or not view.has("parsed_constructor"):
		...
		if has_constructor:
			var function_name = type
			var view_function_name = parsed_constructor.replace("configure", function_name.replace(" ", ""))
			function_code = function_template % [view_function_name, path, parsed_constructor, function_name]
		else:
			var function_name = type
			var view_function_name = function_name.replace(" ", "") + "()"
			function_code = function_template_without_constructor % [view_function_name, path, function_name]
	...
	if new_source_code == source_code:
		print_debug("No changes to View Script on path: %s" % view_script_path)
		return
